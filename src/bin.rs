...
//     let info: Option<InfoResponse> = Info {}.send(&mut scratch, &d)?;
//     println!("{:?}", info.unwrap());
//     Ok(())
// }
...
//     let bininfo: Option<BinInfoResponse> = BinInfo {}.send(&mut scratch, &d)?.unwrap();
...
//     let dmesg: Option<DmesgResponse> = Dmesg {}.send(&mut scratch, &d)?;
//     let dmesg = bininfo.unwrap();
...
fn flash(file: PathBuf, address: u32, d: &HidDevice) -> Result<(), Error> {
    ...
    //how to reveal
    let bininfo = Response::BinInfo(bininfo) = BinInfo {}.send(&mut scratch, &d)?;

    if bininfo.mode != BinInfoMode::Bootloader {
        StartFlash {}.send(&mut [], &d)?;
    }
    ...
    for target_address in (address..top_address).step_by(steps as usize) {
        ...
        let checksums: Option<ChksumPagesResponse> = ChksumPages {
            target_address,
            num_pages,
        }
        .send(&mut scratch, &d)?;
